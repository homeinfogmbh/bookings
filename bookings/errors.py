...
ERRORS = {
    ...
    Bookable.DoesNotExist: lambda _: JSONMessage("No such bookable.", status=404),
    Booking.DoesNotExist: lambda _: JSONMessage("No such booking.", status=404),
    DurationTooLong: lambda error: JSONMessage(
        "Booking duration is too long.", max_duration=error.max_duration, status=400
    ),
    DurationTooShort: lambda error: JSONMessage(
        "Booking duration is too short.", min_duration=error.min_duration, status=400
    ),
    EndBeforeStart: lambda _: JSONMessage("Booking ends before it starts.", status=400),
}
